...
const predictBodyShapes = (answers: QuizAnswers): {
  primary: BodyType;
...
const GuidedBodyShapeFlow = ({
  onComplete,
  onBack
}: GuidedBodyShapeFlowProps) => {
  ...
  const isStepComplete = () => {
    switch (currentStep) {
      case 1:
        return answers.shouldersVsHips !== null;
      case 2:
        return answers.waistDefinition !== null;
      case 3:
  ...
  if (currentStep === "results" && predictions) {
    return <Card className="max-w-2xl mx-auto p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {predictions.subVariations.map((variation, index) => <div key={variation} className="border-2 border-fashion-beige rounded-lg p-6 text-center cursor-pointer hover:border-fashion-coral transition-colors" onClick={() => handleVariationSelect(variation)}>
  ...
  return <Card className="max-w-2xl mx-auto p-8">
      ...
      {currentStep === 1 && <div className="space-y-6">
          <div>
            <h3 className="text-xl font-semibold text-fashion-teal mb-2">When looking in the mirror, which best describes your shoulders compared to your hips?</h3>
            <p className="text-fashion-teal/70 mb-6">Think about the width of your shoulders versus your hip area.</p>
          </div>
          
          <RadioGroup value={answers.shouldersVsHips || ""} onValueChange={value => setAnswers(prev => ({
        ...prev,
        shouldersVsHips: value as any
      }))}>
      ...
      {currentStep === 2 && <div className="space-y-6">
          ...
          <RadioGroup value={answers.waistDefinition || ""} onValueChange={value => setAnswers(prev => ({
        ...prev,
        waistDefinition: value as any
      }))}>
            <div className="flex items-center space-x-2 p-4 border rounded-lg hover:bg-fashion-beige/20">
      ...
      {currentStep === 3 && <div className="space-y-6">
          ...
          <Select value={answers.braSize || ""} onValueChange={value => setAnswers(prev => ({
        ...prev,
        braSize: value
      }))}>
            ...
            <SelectContent>
              {braSizes.map(size => <SelectItem key={size} value={size}>{size}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>}
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <Input type="number" placeholder={answers.heightUnit === "cm" ? "170" : "5'7\""} value={answers.height || ""} onChange={e => setAnswers(prev => ({
              ...prev,
              height: Number(e.target.value)
            }))} className="flex-1" />
                <Select value={answers.heightUnit} onValueChange={value => setAnswers(prev => ({
              ...prev,
              heightUnit: value as "cm" | "inches"
            }))}>
      ...
      <div className="flex justify-between mt-8">
        ...
        <Button onClick={handleNext} disabled={!isStepComplete()} className="min-w-24">
